...
DEFAULT_CONFIG = {
    ...
    "firebase_urls": [u.strip() for u in os.environ.get("FIREBASE_URLS", "https://vyvggvvhhv-default-rtdb.firebaseio.com").split(",") if u.strip()],
...
async def firebase_sniper_worker(speed_delay, scan_mode="deep"):
    try:
        await _firebase_sniper_worker_inner(speed_delay, scan_mode)
    except Exception as e:
        await emit_log(f"🔥 Firebase worker crashed: {type(e).__name__}: {e}", "error")
        state.is_sniping = False

async def _firebase_sniper_worker_inner(speed_delay, scan_mode="deep"):
    init_csvs()
